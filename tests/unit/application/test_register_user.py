...
async def test_register_user_success():
    ...
    result = await uc.execute(
        RegisterUserCommand(email="maria@test.com", password="senha-forte-1")
    )
...
async def test_register_user_with_duplicate_email_raises():
    uc = make_use_case()
    await uc.execute(RegisterUserCommand(email="m@t.com", password="senha-forte-1"))

    with pytest.raises(EmailAlreadyExists):
        await uc.execute(RegisterUserCommand(email="m@t.com", password="outra-senha-1"))
...
async def test_register_user_with_weak_password_raises():
    ...
    with pytest.raises(WeakPassword):
        await uc.execute(RegisterUserCommand(email="m@t.com", password="abc"))
