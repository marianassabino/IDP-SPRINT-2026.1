...
async def test_register_returns_201_and_sets_cookies(client: AsyncClient):
    resp = await client.post(
        "/auth/register",
        json={"email": "maria@test.com", "password": "senha-forte-1"},
    )
...
async def test_register_duplicate_email_returns_409(client: AsyncClient):
    await client.post(
        "/auth/register",
        json={"email": "dup@test.com", "password": "senha-forte-1"},
    )
    resp = await client.post(
        "/auth/register",
        json={"email": "dup@test.com", "password": "outra-forte-2"},
    )
...
async def test_register_weak_password_returns_422(client: AsyncClient):
    resp = await client.post(
        "/auth/register",
        json={"email": "x@t.com", "password": "abc"},
    )
...
async def test_login_success_returns_200(client: AsyncClient):
    await client.post(
        "/auth/register",
        json={"email": "m@t.com", "password": "senha-forte-1"},
    )
